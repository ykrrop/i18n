...
Для извлечения новых сообщений для перевода используйте команду:
...
1. Извлекает все сообщения из исходного кода
2. Обновляет файл `translations.json` новыми сообщениями
3. Сохраняет русский текст как значение по умолчанию

### Компиляция переводов

Для компиляции переводов используйте команду:
...
1. Разбивает переводы по страницам
2. Создает отдельные файлы для каждого языка и страницы
3. Сохраняет скомпилированные переводы в `public/translations-compiled/`

### Структура переводов

- `translations.json` - основной файл с переводами
- `src/page-translation-keys.ts` - список ключей переводов для каждой страницы
- `public/translations-compiled/` - скомпилированные переводы (не коммитить)
...
- Ленивая загрузка переводов по страницам
- Определение языка по приоритетам: URL -> Cookie -> Browser -> Default (en)
- Поддержка регионов в URL (например, ru-RU, ru-BY, ru-KZ)
- Компиляция переводов для уменьшения размера бандла
- Удаление исходных русских текстов из бандла

## Лицензия

© 2024-2025, ООО «Альт». Все права защищены
